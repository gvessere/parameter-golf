...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.num_skip_weights=min(len(self.encoder_indices),len(self.decoder_indices));self.skip_weights=nn.Parameter(torch.ones(self.num_skip_weights,h.model_dim,dtype=torch.float32));self.skip_gates=nn.Parameter(torch.zeros(self.num_skip_weights,h.model_dim,dtype=torch.float32))if h.skip_gates_enabled else None;self.mtp_horizons=h.mtp_horizons;self.mtp_weight=h.mtp_weight;self.mtp_use_query=h.mtp_use_query;self.mtp_use_emb=h.mtp_use_emb;self.contrastive_weight=h.contrastive_weight;self.contrastive_temp=h.contrastive_temp;self.contrastive_bos_id=h.contrastive_bos_id;self.contrastive_max_docs=h.contrastive_max_docs;self.contrastive_k_samples=h.contrastive_k_samples;self.horizon_emb=nn.Parameter(torch.zeros(h.mtp_horizons,h.model_dim,dtype=torch.float32))if h.mtp_horizons>0 and h.mtp_use_emb else None;self.horizon_query=nn.Parameter(torch.zeros(h.mtp_horizons,h.model_dim,dtype=torch.float32))if h.mtp_horizons>0 and h.mtp_use_query else None;self.loop_end_idx=h.loop_end;self._init_weights()
	...
	def _doc_contrastive_loss(self,x,input_ids):
		...
		# View 1: mean of K randomly sampled positions from each doc
		# View 2: mean of another K randomly sampled positions (non-overlapping) from same doc
		# fullgraph disabled to allow dynamic per-doc random sampling
		B,T,D=x.shape;M=self.contrastive_max_docs;K=self.contrastive_k_samples;x_f=x.float()
		bos_mask=(input_ids==self.contrastive_bos_id)
		doc_idx=(bos_mask.float().cumsum(dim=1)-1).long().clamp(0,M-1)
		v1=torch.zeros(B*M,D,device=x.device,dtype=torch.float32)
		v2=torch.zeros(B*M,D,device=x.device,dtype=torch.float32)
		valid=torch.zeros(B*M,device=x.device,dtype=torch.bool)
		for b in range(B):
			for m in range(M):
				pos=(doc_idx[b]==m).nonzero(as_tuple=True)[0]
				# doc must start with BOS and have enough tokens for 2 non-overlapping views
				if pos.numel()<2*K or not bos_mask[b,pos[0]]:continue
				perm=torch.randperm(pos.numel(),device=x.device)
				idx1=pos[perm[:K]];idx2=pos[perm[K:2*K]]
				v1[b*M+m]=x_f[b,idx1].mean(0)
				v2[b*M+m]=x_f[b,idx2].mean(0)
				valid[b*M+m]=True
...
def train_model(h,device,val_data):
	base_model=GPT(h).to(device).bfloat16();restore_fp32_params(base_model);compiled_model=torch.compile(base_model,dynamic=False,fullgraph=h.contrastive_weight==0.)
