...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.num_skip_weights=min(len(self.encoder_indices),len(self.decoder_indices));self.skip_weights=nn.Parameter(torch.ones(self.num_skip_weights,h.model_dim,dtype=torch.float32));self.skip_gates=nn.Parameter(torch.zeros(self.num_skip_weights,h.model_dim,dtype=torch.float32))if h.skip_gates_enabled else None;self.mtp_horizons=h.mtp_horizons;self.mtp_weight=h.mtp_weight;self.horizon_emb=nn.Parameter(torch.zeros(h.mtp_horizons,h.model_dim,dtype=torch.float32))if h.mtp_horizons>0 else None;self.horizon_query=nn.Parameter(torch.zeros(h.mtp_horizons,h.model_dim,dtype=torch.float32))if h.mtp_horizons>0 else None;self._init_weights()
	...
	def forward(self,input_ids,target_ids):
		...
		for k in range(self.mtp_horizons):
			step=k+2;e_k=self.horizon_emb[k];q_k=self.horizon_query[k]
			scores=(x.float()@q_k.float().unsqueeze(-1)).squeeze(-1)/(D**.5)
			scores=scores.unsqueeze(1).expand(-1,T,-1).masked_fill(causal_mask,float('-inf'))
			x_k=self.final_norm((torch.softmax(scores,dim=-1)@x).to(x.dtype)+e_k)
