...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.num_skip_weights=min(len(self.encoder_indices),len(self.decoder_indices));self.skip_weights=nn.Parameter(torch.ones(self.num_skip_weights,h.model_dim,dtype=torch.float32));self.skip_gates=nn.Parameter(torch.zeros(self.num_skip_weights,h.model_dim,dtype=torch.float32))if h.skip_gates_enabled else None;self.mtp_horizons=h.mtp_horizons;self.mtp_weight=h.mtp_weight;self.mtp_use_query=h.mtp_use_query;self.mtp_use_emb=h.mtp_use_emb;self.horizon_emb=nn.Parameter(torch.zeros(h.mtp_horizons,h.model_dim,dtype=torch.float32))if h.mtp_horizons>0 and h.mtp_use_emb else None;self.horizon_query=nn.Parameter(torch.zeros(h.mtp_horizons,h.model_dim,dtype=torch.float32))if h.mtp_horizons>0 and h.mtp_use_query else None;self._init_weights()
