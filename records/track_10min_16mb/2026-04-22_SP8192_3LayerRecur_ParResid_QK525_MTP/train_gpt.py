...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.num_skip_weights=min(len(self.encoder_indices),len(self.decoder_indices));self.skip_weights=nn.Parameter(torch.ones(self.num_skip_weights,h.model_dim,dtype=torch.float32));self.skip_gates=nn.Parameter(torch.zeros(self.num_skip_weights,h.model_dim,dtype=torch.float32))if h.skip_gates_enabled else None;self.tc_weight=h.tc_weight;self.tc_temp=h.tc_temp;self.tc_mode=h.tc_mode;self.tc_layer=h.tc_layer;self.tc_n_pairs=h.tc_n_pairs;self.loop_end_idx=h.loop_end
		self.mtp_weight=h.mtp_weight;self.mtp_horizons=h.mtp_horizons;self.mtp_type=h.mtp_type;self.mtp_heads=None;self.mtp_block=None
		if h.mtp_weight>0.:
			if h.mtp_type=='mlp':self.mtp_heads=nn.ModuleList([nn.Linear(h.model_dim,h.model_dim,bias=False)for _ in range(h.mtp_horizons)])
			else:self.mtp_block=Block(h.model_dim,h.num_heads,h.num_kv_heads,h.mlp_mult,h.rope_base,h.qk_gain_init,h.train_seq_len,layer_idx=h.num_layers,ln_scale=h.ln_scale)
		self._init_weights()
	...
	def _temporal_contrast_loss(self,x):
		...
		# repulsive: cosine similarity loss pushing h_t away from h_{t+1}
		B,T,D=x.shape;x_f=x.float()
	...
	def forward(self,input_ids,target_ids):
		if not self.training or(self.tc_weight==0. and self.mtp_weight==0.):
		...
		elif self.tc_weight>0. and self.tc_layer=='pre_logits':loss=loss+self.tc_weight*self._temporal_contrast_loss(x_ar)
		if self.mtp_weight>0.:loss=loss+self.mtp_weight*self._mtp_loss(x,target_ids)
		return loss,ar_loss
...
class Optimizers:
	def __init__(self,h,base_model):
		...
		if base_model.mtp_heads is not None:
			mtp_named=list(base_model.mtp_heads.named_parameters());matrix_params.extend([p for(n,p)in mtp_named if p.ndim==2]);scalar_params.extend([p for(n,p)in mtp_named if p.ndim<2])
		elif base_model.mtp_block is not None:
			mtp_named=list(base_model.mtp_block.named_parameters());matrix_params.extend([p for(n,p)in mtp_named if p.ndim==2 and not any(pat in n for pat in CONTROL_TENSOR_NAME_PATTERNS)]);scalar_params.extend([p for(n,p)in mtp_named if p.ndim<2 or any(pat in n for pat in CONTROL_TENSOR_NAME_PATTERNS)])
...
def train_model(h,device,val_data):
	...
	def step_fn(step,lr_scale):
		...
		train_loss/=h.grad_accum_steps;ar_loss_total/=h.grad_accum_steps;tc_loss_approx=(train_loss-ar_loss_total)/max(h.tc_weight,1e-8)if h.tc_weight>0 else train_loss.new_zeros(());frac=min(step/h.muon_momentum_warmup_steps,1.)if h.muon_momentum_warmup_steps>0 else 1.;muon_momentum=(1-frac)*h.muon_momentum_warmup_start+frac*h.muon_momentum
		...
		optimizers.step();return train_loss,ar_loss_total,tc_loss_approx
	if h.warmup_steps>0:
	...
	while True:
		...
		train_loss,ar_loss_step,tc_loss_approx=step_fn(step,scale)
		with torch.no_grad():
		...
		if should_log_train:
			...
			con_str=f" tc_loss: {tc_loss_approx.item():.4f}"if h.tc_weight>0 else""
