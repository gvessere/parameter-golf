...
class Hyperparameters:
    ...
    bottleneck_l_cycles = int(os.environ.get("BOTTLENECK_L_CYCLES", 2))
    bottleneck_expansion = int(os.environ.get("BOTTLENECK_EXPANSION", 2))
    bottleneck_conv_kernel = int(os.environ.get("BOTTLENECK_CONV_KERNEL", 2))
    use_hyper_connections = bool(int(os.environ.get("USE_HYPER_CONNECTIONS", "0")))
    hyper_num_streams = int(os.environ.get("HYPER_NUM_STREAMS", 4))
...
# -----------------------------
# URM BOTTLENECK MODULES
# -----------------------------

class CausalConvSwiGLU(nn.Module):
    """SwiGLU FFN with causal depthwise 1D convolution, adapted from URM's ConvSwiGLU."""

    def __init__(self, dim: int, expansion: int, conv_kernel: int = 2):
        super().__init__()
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        y = F.silu(gate) * up
        # Causal conv: left-pad so position t only sees t-(K-1)..t
        y_t = y.transpose(1, 2)  # [B, hidden, T]
        y_t = F.pad(y_t, (self.conv_kernel - 1, 0))
        y_t = F.conv1d(
            y_t,
            self.dwconv.weight.to(y_t.dtype),
            self.dwconv.bias.to(y_t.dtype) if self.dwconv.bias is not None else None,
            groups=self.dwconv.groups,
        )
        y = F.silu(y_t).transpose(1, 2).contiguous()
        return self.down(y)


class CayleyOrthogonalHyperConnection(nn.Module):
    """Multi-stream gated residual with Cayley orthogonal mixing, from URM."""

    def __init__(self, dim: int, num_streams: int = 4, tau: float = 1.0,
        ...
        self.fused_proj = CastedLinear(dim, 3 * num_streams * num_streams, bias=True)
        self.register_buffer("_I", torch.eye(num_streams, dtype=torch.float32), persistent=False)

    def _iterative_cayley(self, raw: Tensor) -> Tensor:
    ...
    def forward(self, x: Tensor, sublayer_fn) -> Tensor:
...
class BottleneckBlock(nn.Module):
    """A single block for the URM recurrent bottleneck stage."""
    ...
    def __init__(
        ...
        self.mlp_norm = RMSNorm()
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init)
        self.mlp = CausalConvSwiGLU(dim, expansion, conv_kernel)
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.use_hyper_connections:
            x = self.hc_attn(x, lambda h: self.attn(self.attn_norm(h)))
...
class GPT(nn.Module):
    def __init__(
        ...
        bottleneck_h_cycles: int = 1,
        bottleneck_l_cycles: int = 2,
        bottleneck_expansion: int = 2,
        bottleneck_conv_kernel: int = 2,
        use_hyper_connections: bool = False,
        hyper_num_streams: int = 4,
    ):
        super().__init__()
        ...
        self.bottleneck_h_cycles = bottleneck_h_cycles
        self.bottleneck_l_cycles = bottleneck_l_cycles
        ...
        self.bottleneck = nn.ModuleList(
            [
                BottleneckBlock(
                    ...
                    conv_kernel=bottleneck_conv_kernel,
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        bottleneck_h_cycles=args.bottleneck_h_cycles,
        bottleneck_l_cycles=args.bottleneck_l_cycles,
        bottleneck_expansion=args.bottleneck_expansion,
        bottleneck_conv_kernel=args.bottleneck_conv_kernel,
    ...
    # - matrix params in transformer blocks + bottleneck use MATRIX_LR via Muon
    # - vectors/scalars use SCALAR_LR via Adam
    all_block_named_params = list(base_model.blocks.named_parameters()) + list(base_model.bottleneck.named_parameters())
    matrix_params = [
        p
    ...
    bn_depth = args.bottleneck_h_cycles * args.bottleneck_l_cycles * args.bottleneck_blocks
    log0(
        f"bottleneck: blocks:{args.bottleneck_blocks} h_cycles:{args.bottleneck_h_cycles} "
        f"l_cycles:{args.bottleneck_l_cycles} effective_depth:{bn_depth} "
        f"expansion:{args.bottleneck_expansion} conv_kernel:{args.bottleneck_conv_kernel} "
        f"hyper_connections:{args.use_hyper_connections}"
    )
