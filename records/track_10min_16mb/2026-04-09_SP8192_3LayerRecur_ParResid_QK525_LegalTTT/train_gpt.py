...
class GPT(nn.Module):
	...
	def forward(self,input_ids,target_ids):
		logits=self.forward_logits(input_ids);V=logits.size(-1);flat=logits.reshape(-1,V).float()
		ce=F.cross_entropy(flat,target_ids.reshape(-1),reduction='mean')
		if not self.training or self.bigram_possible is None:return ce
		prev1=input_ids.reshape(-1)
		impossible=(~self.bigram_possible[prev1])
		if self.trigram_lookup is not None:
			padded=F.pad(input_ids,(1,0));prev2=padded[:,:-1].reshape(-1)
			row_idx=self.trigram_lookup[prev2,prev1];has_tri=(row_idx>=0)
			tri_impossible=~self.trigram_possible[row_idx.clamp(min=0)]
			impossible=torch.where(has_tri.unsqueeze(-1),tri_impossible,impossible)
		penalty=F.relu(flat+self.ngram_penalty_margin).pow(2)*impossible.float()
		return ce+self.ngram_penalty_weight*penalty.mean()
...
def train_model(h,device,val_data):
	...
	if h.ngram_penalty_weight>0.:
		def _load(p,tag):
			if os.path.exists(p):t=torch.load(p,map_location=device,weights_only=True);log(f'ngram:{tag} loaded {tuple(t.shape)} {t.dtype}');return t
			else:log(f'ngram:{tag} not found at {p}');return None
		bp=_load(h.bigram_possible_path,'bigram_possible')
		if bp is not None:
			base_model.register_buffer('bigram_possible',bp,persistent=False)
			tp=_load(h.trigram_possible_path,'trigram_possible')
			tl=_load(h.trigram_lookup_path,'trigram_lookup')
			if tp is not None:base_model.register_buffer('trigram_possible',tp,persistent=False)
			if tl is not None:base_model.register_buffer('trigram_lookup',tl,persistent=False)
