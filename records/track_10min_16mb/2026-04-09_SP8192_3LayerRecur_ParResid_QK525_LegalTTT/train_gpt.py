...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.num_skip_weights=min(len(self.encoder_indices),len(self.decoder_indices));self.skip_weights=nn.Parameter(torch.ones(self.num_skip_weights,h.model_dim,dtype=torch.float32));self.skip_gates=nn.Parameter(torch.zeros(self.num_skip_weights,h.model_dim,dtype=torch.float32))if h.skip_gates_enabled else None;self._init_weights();self.multi_step_weight=h.multi_step_weight;self.multi_step_k=h.multi_step_k;self.multi_step_decay=h.multi_step_decay;self.token_dropout_rate=h.token_dropout_rate;self.mars_weight=h.mars_weight;self.mars_block_size=h.mars_block_size;self.register_buffer('multi_step_scale',torch.tensor(1.0),persistent=False)
	...
	def forward(self,input_ids,target_ids):
		...
		if not self.training:return ce1
		aux=ce1.new_zeros(())
		if self.multi_step_weight>0. and self.multi_step_k>=2:
		...
		if self.mars_weight>0.:
			T=input_ids.size(1);pos=torch.arange(T,device=input_ids.device);bm=(pos%self.mars_block_size)!=0
			noisy_ids=input_ids.masked_fill(bm.unsqueeze(0),0);nlogits=self.forward_logits(noisy_ids)
			flat_nlogits=nlogits.reshape(-1,V).float();flat_tgt=target_ids.reshape(-1)
			bm_flat=bm.unsqueeze(0).expand(input_ids.size(0),-1).reshape(-1).float()
			aux=aux+self.mars_weight*(F.cross_entropy(flat_nlogits,flat_tgt,reduction='none')*bm_flat).sum()/bm_flat.sum()
		if self.multi_step_weight<=0. and self.mars_weight<=0.:return ce1
		return ce1+aux,ce1
...
def train_model(h,device,val_data):
	...
	while True:
		...
		if h.multi_step_weight>0.:phase_frac=max(0.,(frac-h.multi_step_phase_out)/(1.-h.multi_step_phase_out+1e-8));base_model.multi_step_scale.fill_(max(0.,1.-phase_frac))
		...
		if should_log_train:tok_per_sec=step*h.train_batch_tokens/(approx_training_time_ms/1e3);ce1_str=f" train_ce1: {train_ce1.item():.4f}" if h.multi_step_weight>0. or h.mars_weight>0. else "";log(f"{step}/{h.iterations} train_loss: {train_loss.item():.4f}{ce1_str} train_time: {approx_training_time_ms/60000:.1f}m tok/s: {tok_per_sec:.0f}")
...
def train_and_eval(h,device):
	...
	if bool(int(os.environ.get('SKIP_QUANTIZATION','0'))):return
